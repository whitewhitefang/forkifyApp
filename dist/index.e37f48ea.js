...
var _ingredientsView = require("./views/ingredientsView");
var _ingredientsViewDefault = parcelHelpers.interopDefault(_ingredientsView);
...
const searchRecipes = async function() {
    try {
        ...
        if (!query || query === "") _resultsViewDefault.default.renderError("You did not write any seeking word");
        else {
...
const controlIngredients = function(type) {
    if (type === "add") _ingredientsViewDefault.default.showIngredients();
    else if (type === "remove") _ingredientsViewDefault.default.removeProduct();
};
const controlSorting = function(type) {
    _model.sortResults(_model.state.search.searchResults, type);
    _resultsViewDefault.default.render(_model.state.search.searchResults);
    _resultsViewDefault.default.render(_model.getSearchResultsPage(1));
    _paginationViewDefault.default.render(_model.state.search);
};
const controlProducts = function() {
    if (_model.state.products.length > 0) _productsViewDefault.default.render(_model.state.products);
};
const controlAddProducts = function() {
    _model.addProduct();
    _productsViewDefault.default.render(_model.state.products);
    _recipeViewDefault.default.update(_model.state.recipe);
};
...
const controlRemoveAllProducts = function() {
    _model.removeAllProducts();
    _productsViewDefault.default.render(_model.state.products);
    _recipeViewDefault.default.update(_model.state.recipe);
};
const init = function() {
    _recipeViewDefault.default.addHandlerRender(controlRecipes);
    _searchViewDefault.default.addHandlerSearch(searchRecipes);
    _paginationViewDefault.default.addHandlerClick(controlPagination);
    _recipeViewDefault.default.addHandlerChangeServings(controlServings);
    ...
    _addRecipeViewDefault.default.addHandlerUpload(controlUploadRecipe);
    _resultsViewDefault.default.addHandlerSort(controlSorting);
    _ingredientsViewDefault.default.addProductHandler(controlIngredients);
    _productsViewDefault.default.addHandlerRender(controlProducts);
    _productsViewDefault.default.addHandlerRemoveProduct(controlRemoveProduct);
    _productsViewDefault.default.addHandlerRemoveAll(controlRemoveAllProducts);
    _recipeViewDefault.default.addHandlerAddProduct(controlAddProducts);
};
init();

},{"core-js/modules/web.immediate.js":"49tUX","./model":"Y4A21","./views/recipeView":"l60JC","./views/searchView":"9OQAM","regenerator-runtime/runtime":"dXNgZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./views/resultsView":"cSbZE","./views/paginationView":"6z7bi","./views/bookmarksView":"4Lqzq","./views/addRecipeView":"i6DNj","./config":"k5Hzs","./views/ingredientsView":"hstKP","./views/productsView":"cfXP5"}],"49tUX":[function(require,module,exports) {
...
parcelHelpers.export(exports, "getNutrition", ()=>getNutrition
);
...
parcelHelpers.export(exports, "persistProducts", ()=>persistProducts
);
parcelHelpers.export(exports, "addProduct", ()=>addProduct
);
parcelHelpers.export(exports, "removeProduct", ()=>removeProduct
);
parcelHelpers.export(exports, "removeAllProducts", ()=>removeAllProducts
);
...
const createRecipeObject = function(realRecipe) {
    return {
        ...
        /**optional key field */ ...realRecipe.data.recipe.key && {
            key: realRecipe.data.recipe.key
        }
    };
};
const getNutrition = async function(recipe) {
    try {
        const recipeOnSpoon = await _helpers.ajax(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${_config.SPOON_API_KEY}&query=${recipe}&number=1`);
        ...
        if (!id) {
            ...
            const widjetRequest = await fetch(`https://api.spoonacular.com/recipes/${id}/nutritionLabel?apiKey=${_config.SPOON_API_KEY}`, {
                method: "GET",
                headers: {
                    "Content-Type": "text/html"
                }
            });
            if (!widjetRequest.ok) throw new Error('Problem with getting widjet');
...
const loadRecipe = async function(id) {
    try {
        const realRecipe = await _helpers.ajax(`${_config.API_URL}${id}?key=${_config.API_KEY}`);
...
const loadSearchResults = async function(searchedRecipe) {
    try {
        state.search.query = searchedRecipe;
        state.search.page = 1;
        const results = await _helpers.ajax(`${_config.API_URL}?search=${state.search.query}&key=${_config.API_KEY}`);
...
const sortResults = function(results, type) {
    const resultsArray = [
        ...results
    ];
    if (type === "none") state.search.searchResults = state.search.initSearchResults;
    if (type === "time") state.search.searchResults = state.search.initSearchResults;
    if (type === "name") {
...
const updateServings = function(nums) {
    state.recipe.ingredients.forEach((ingr)=>{
        if (+ingr.quantity) ingr.quantity = (Math.abs(+ingr.quantity) / state.recipe.servings * nums).toFixed(1);
    });
...
const deleteBookmark = function(recipeId) {
    state.bookmarks.splice(state.bookmarks.findIndex((rec)=>rec.id === recipeId
    ), 1);
...
const addProduct = function() {
    if (state.products.length === 0) state.products = [
        ...
        state.recipe.ingredients.forEach((product)=>{
            ...
            } else newArrOfProducts.push(product);
        });
        state.products = [
            ...newArrOfProducts
        ];
...
const uploadRecipe = async function(data) {
    try {
        ...
        ingredients.forEach((ingr)=>{
            if (!ingrArr[+ingr[0].substring(11, 12)]) ingrArr[+ingr[0].substring(11, 12)] = [];
        ...
        for(let i = 0; i < ingrArr.length; i++){
            ...
            ingrArr[i].forEach((ingr)=>{
                if (ingr[0].includes('product')) ingredient.description = ingr[1];
                if (ingr[0].includes('quantity')) ingredient.quantity = ingr[1];
                if (ingr[0].includes('unit')) ingredient.unit = ingr[1];
            });
        ...
        const uploadRequest = await _helpers.ajax(`${_config.API_URL}?key=${_config.API_KEY}`, recipe);
        if (uploadRequest.status === "success") {
            state.recipe = createRecipeObject(uploadRequest);
            addBookmark(state.recipe);
        } else throw new Error("Sorry, something got wrong");
    } catch (err) {
        throw err;
    }
};
const init = function() {
    ...
    if (storage.length) // localStorage.clear();
    state.bookmarks = JSON.parse(storage);
    const products = localStorage.getItem('products') || [];
    if (products.length) state.products = JSON.parse(products);
};
init();
...
parcelHelpers.export(exports, "MODAL_CLOSE_MILSECS", ()=>MODAL_CLOSE_MILSECS
);
parcelHelpers.export(exports, "SPOON_API_KEY", ()=>SPOON_API_KEY
);
const API_URL = `https://forkify-api.herokuapp.com/api/v2/recipes/`;
const TIMEOUT_SEC = 10;
const API_KEY = '027304e4-069e-4a19-91b9-589aa36ef008';
const SEARCH_RESULTS_PER_PAGE = 10;
const MODAL_CLOSE_MILSECS = 1500;
const SPOON_API_KEY = 'd1630a99400c431093d907f9f7cb7957';
...
const ajax = async function(url, uploadData) {
    try {
        ...
        if (!request.ok) throw new Error(`Some server problem - ${request.statusText}`);
...
class RecipeView extends _viewDefault.default {
    ...
    addHandlerChangeServings(handler) {
        this._parentElement.addEventListener('click', function(event) {
            ...
            if (+changeto > 0 && +changeto <= 10) handler(+changeto);
        });
    }
    ...
    _generateMarkup() {
      ...
      <div class="recipe__ingredients">
        ...
        <ul class="recipe__ingredient-list">
          ${this._data.ingredients.map((ingr)=>this._generateMarkupIngredient(ingr)
        ).join("")}          
        </ul>
      </div>
      ...
      <li class="recipe__ingredient">
        ...
        <div class="recipe__description">   
          <span class="recipe__quantity">
            ${+ingr.quantity && ingr.quantity !== " " ? _fractyDefault.default(Math.abs(+ingr.quantity)) : ""}
          </span>
...
},{"./view":"bWlJ9","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","fracty":"hJO4d"}],"bWlJ9":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class View {
    _data;
    _icons = _iconsSvgDefault.default;
...
class ResultsView extends _viewDefault.default {
    ...
    addHandlerSort(handler) {
        this._sortingInputs.addEventListener('change', function(event) {
            if (!event.target.closest('.sorting_button')) return;
            else handler(event.target.value);
        });
    }
}
...
class AddRecipeView extends _viewDefault.default {
    ...
    toggleWindow() {
        ...
        if (this._window.classList.contains("hidden")) document.body.style.overflow = "auto";
...
},{"./view":"bWlJ9","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hstKP":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./view");
var _viewDefault = parcelHelpers.interopDefault(_view);
class IngredientsView extends _viewDefault.default {
    constructor(){
        super();
...
exports.default = new IngredientsView();

},{"./view":"bWlJ9","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"cfXP5":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./view");
var _viewDefault = parcelHelpers.interopDefault(_view);
var _iconsSvg = require("../../img/icons.svg");
var _iconsSvgDefault = parcelHelpers.interopDefault(_iconsSvg);
class ProductsView extends _viewDefault.default {
    ...
    _generateMarkup() {
        return `
      ${this._data.map((product)=>this._generateProduct(product)
        ).join("")}
...
},{"./view":"bWlJ9","../../img/icons.svg":"cMpiy","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["9GjUt","aenu9"], "aenu9", "parcelRequire3a11")
